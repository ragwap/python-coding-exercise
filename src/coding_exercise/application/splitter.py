...
class Splitter:
    ...
    def __validate(self, cable: Cable, times: int):
        ...
        # Checks if times and cable lengths are within the specified ranges
        ...
        # Checks if a cable can be split into equal/valid lengths with the given times
    ...
    def split(self, cable: Cable, times: int) -> list[Cable]:
        ...
        gcd = math.gcd(*split_arr) # The greatest common divisor should be the length of each cable
        res_range = cable.length // gcd # Calculating the number of cables

        # Find the number of digits in the maximum value to determine zeros to fill
